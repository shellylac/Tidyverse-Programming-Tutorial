...
grouped_mean_dots <- function(data, summary_column, ...) {
  data %>%
    group_by(...) %>%
    summarise("mean_{{summary_column}}" := mean({{ summary_column }}, na.rm = T), .groups="keep")
}
...
> #### SOLUTION C - for STRING INPUT can use string vector helpers: `all_of()` / `any_of()` or scoped dplyr verbs e.g. `summarise_at()`
>
...
It doesn't work because `columns[[col]]`, a string representing the name of a
column in gapminder, is not recognised as a data variable within the context of
the `mean` function. Now we can try again using the `.data` pronoun.
...
It worked but it's ugly and we have an awkward output format. We can improve
this by using the tidy selection function `across` to drop the for loop:
```{r for_loop_replaced}
columns <- names(gapminder)[4:6]
gapminder %>%
