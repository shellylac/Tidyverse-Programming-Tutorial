...
-   NOTE that you can only pass one set of inputs to the `(...)` - so you
    can't use it to specify group variables *and* summary variables!

> #### SOLUTION C - use `across()`
>
> [Ideal for]{.underline}: Arguments which specify groups of (unquoted)
> data variables, that you want to pass to a tidyverse function that
> does not implement <tidy-select> (so you can't use `{{ }}`). You may not
> want to pass the dots (e.g. if you want to use them for another
...
>
> [Ideal for]{.underline}: Arguments which specify groups of QUOTED
> data variables
>
...
#### Not if you can select the columns you need using across() with tidy-select
  ...
  -   across(where(is.numeric), sd) --\> map over columns selected by condition

  -   across(c(country, continent), to_upper) --\> map over a custom selection
