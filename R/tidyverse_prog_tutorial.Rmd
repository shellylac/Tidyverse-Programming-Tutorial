...
column name that our new mean column is given:
...
for(col in columns){
  gapminder %>% 
    summarise(across(col, ~ mean(.data[[col]]), .names = "mean_{col}")) %>% 
    print()
}
#Note that here we use the .names = "" option provided with across() to get bespoke names.
```
...
-   When you have an a string input, you need to use `all_of()` or
    `any_of()` depending on whether you want the function to error if a
...
-   And if you have string input (or don't want to propagate data
    masking):
...
functions. So we can plot our grouped means table
