...
`select()`. In simple cases the solution is obtained with curly-curly
`{{ }}`:

> #### Solution 1:
>
> [Unquoted arguments + tidy-select]{.underline}: Embrace the function
> input vars using `{{ }}` when using tidyverse functions with
> tidy-select:
...
> `{{ }}` to combine the function input vars when using tidyverse
