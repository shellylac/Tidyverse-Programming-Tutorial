...
`relocate()`, `rename_with()`, and `select()`
...
Behind the scenes, dplyr verbs achieve this by [capturing the blueprint
of your code, and resuming its evaluation inside the data
...
> # EXERCISE BREAKOUT
>
> # Please go to `~R/worksheet/prog-todyverse-worksheet.Rmd` and complete Exercises 1 & 2
...
# Please go to `~R/worksheet/prog-todyverse-worksheet.Rmd` and complete Exercise 3
...
> #### UNQUOTED ARUGMENTS as function inputs
>
...
> #### SINGLE QUOTED ARUGMENT as function inputs
>
> Use `.data[[ var ]]` pronoun as described above to subset the data
> set.

```{r slice_example_1}
slice_string <- function(data, select_column, rows) {
  data %>%
    select(.data[[ select_column ]]) %>%
    slice(rows) %>% 
    arrange(desc(.data[[ select_column ]]))
}

slice_string(gapminder, "pop", 1:3)

```

------------------------------------------------------------------------

## What about vector string input and multiple input variables in general? See `Extras.Rmd`
