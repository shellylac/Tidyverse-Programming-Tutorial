...
**Tidyverse functions deal with data variables by using non-standard
(tidy) evaluation**
...
We want our functions to be like that, too! So how does the tidyverse do
this? The answer lies in non-standard evaluation.
...
plenty of difficulties along the way! Here are some common problems and
...
# An object in your workspace (which you want to use) has the same name as a 
# dataset column. 
add_something <- function(data, col) {
  n <- sample.int(200, size = 1)
  data %>% mutate(y = {{ col }} * z + n)
}

data1 <- data.frame(x = 100, z = 10)
data2 <- data.frame(x = 100, z = 10, n = "argh")
...
add_something <- function(data, col) {
  n <- sample.int(200, size=1)
  z <- 20
  data %>% mutate(y = {{ col }} * .data$z + .env$n)
}
...
Sometimes you will need to interpret strings as data variable names. For
example, if the name of the required data variable is read in from a
separate file or from user input, and held within an environment
variable, you may still want to use it to select a data variable!
...
You might want to allow the function user to select sets of (not
necessarily sequential) columns, as they can when using tidyverse
select(). Once again the solution is usually easy with curly-curly:

> #### Solution 1:
>
> Unquoted arguments: Embrace the function input vars using `{{ }}` -
> when the input is not a string input
...
slice_fun(gapminder, continent, where(is.double), 1)

```

-   Alternatively - if you have string inputs (or don't want to propagate
    data masking), you can accept a set of column names as a vector:

> #### Solution 2:
>
> Quoted arguments: Use all_of() - when the input is a list of strings
> giving the variable names

```{r}
slice_character_subset <- function(data, columns, row_start, row_end) {
  data %>%
    select(all_of(columns)) %>%
    slice(row_start:row_end)
}
slice_character_subset(gapminder, c("country", "year", "pop"), 1, 10)
