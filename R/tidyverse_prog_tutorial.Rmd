...
`select()`. Once again the solution is usually obtained with curly-curly
`{{ }}`:

> #### Solution 1:
>
> [Unquoted arguments]{.underline}: Embrace the function input vars
> using `{{ }}` when the input is not a string input:
...
> #### Solution 2:
>
> [Quoted arguments]{.underline}: Use `all_of()` when the input is a
> list of strings giving the variable names (or don't want to propagate
> data masking):
