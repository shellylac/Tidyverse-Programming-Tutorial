...
-   Basics of programming, tidy-style
...
-   Note that `.data` is not a data frame; it's a special construct, a
    ...
    `.data[[var]]` (in a grouped dataset it represents the subset for the 
    current group). Don't expect other functions to work with it.

> If you want to assign a name using string inputs (either function arguments or
> environment variables) then use `:=` + `{ }` (***single curly brackets***)
...
## What about ggplot? - see `Extras.Rmd`
