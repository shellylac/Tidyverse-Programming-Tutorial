...
```{r load_prelims, message=FALSE, warning=FALSE}
...
Check your answer by running the next cell!
...
The `group_index` function shown below does not work. Your task is to identify why and fix it so that the function works as described:

    #' Group data by a given column and add a row index in a new column
    #'
    #' @param data A data frame
    #' @param group_col A column to group by. Column name should be unquoted.
    #' @param out_col A new column name for the index column. Column name should be unquoted.
    #'
    #' @return A data frame with one additional column containing a sequential index for each row
    #' within its group as specified in `group_col`. The index variable is given the name specified 
    #' by `out_col`.
    #' @export
    #'
    #' @examples
    #' library(palmerpenguins)
    #' group_index(penguins, species, sp_idx)
    group_index <- function(data, group_col, out_col) {
        data %>% 
            group_by({{ group_col }}) %>% 
            mutate({{ out_col }} = row_number())
    }
...
### Exercise 4 (advanced - see Extras.Rmd)

Take the function you fixed in exercise 1 and **rename it** `get_rectangle2`. Now make it work better with <tidy-select> syntax by added pass the dots, `...`, in place of `column_range`. When you do this, play with inputs to this function, what can you do with `...` as opposed to using a named argument? *Hint: try selecting columns that are not beside each other.* What can't you do? *Hint: think about function argument defaults.*
...
Write a function called `select_and_arrange` whose documentation we provide below. This function should use embracing with `{{` and pass the dots `...` syntax to accomplish its job.
...
This is your last worksheet question! This one is a choose your own adventure. Write any function that takes a data frame, and an unquoted column name. Use either `{{` or `...` (or both!) to deal with the unquoted column name(s). All you need to do is make sure it works. Be creative!
...
Congratulations! You are done with the worksheet!!! Pat yourself on the back!
